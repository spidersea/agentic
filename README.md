...
- 🔄 规则自进化协议（规则触发分析、退化检测、Spa Day 清理）
...
| ⭐⭐ | `/evolve` | 规则进化清理 | 健康检查 → 规则触发分析（从未触发/频繁违反/有效防止）→ 去重 → 冲突检测 → 淘汰 → 重组 | 建议每 2-4 周或规则 >20 条时执行 |
...
| ⭐⭐ | `/finish` | 分支完成 | 验证测试 → 展示 4 选项（本地合并/创建 PR/保留/丢弃）→ 执行选择 → 清理 worktree | 在 `/new-feature` 闭环验收后自动链接 |
