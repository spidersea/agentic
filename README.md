...
| 你输入 | AI 会做什么 | 打个比方 |
|---|---|---|
| `/new-feature` | 完整流程开发新功能 | 就像写作文：先审题 → 列提纲 → 写正文 → 检查错别字 |
| `/debug` | 找 Bug 并修复 | 就像医生看病：先问哪里不舒服 → 做检查 → 确诊 → 开药 |
| `/review` | 检查代码质量 | 就像作文互改：同学 A 找问题 → 同学 B 反驳 → 老师做裁判 |
| `/test` | 给代码写测试 | 就像出练习题：写几道题来检验代码写得对不对 |
| `/tdd` | 先写测试再写代码 | 就像先出试卷再学知识——知道要考什么，学习更有方向 |
...
| `continuous-learning` | 怎么从经验中学习 | 📦 **新增** 每次会话都生效 |
| `hooks-lifecycle` | 自动保存/恢复状态 | 📦 **新增** 每次会话都生效 |
| `doc-lookup` | 查资料的正确方法 | 📦 **新增** 调研阶段 |
...
└── .agent/
    ├── skills/                ← 📕 教科书（27 本）
    ├── workflows/             ← 📋 标准流程（21 个，用 /命令 触发）
    ├── agents/                ← 🤖 专职助手（5 个）
    ├── rules/                 ← 📏 规矩（编码风格、测试、安全等）
