...
默认模式下 Adversary 使用通用 8 维度攻击。当以下知识库存在时，**必须优先使用知识库驱动的模式匹配审查**：

1. **加载优先级**:
   - `.agent/skills/quality-patterns/SKILL.md` → 通用质量模式库（代码质量 + 性能 + 测试 + 安全）
   - `.agent/state/captured-patterns/` → 项目级积累模式
   - `.agent/state/memory-palace/failure-patterns.jsonl` → 历史失败经验

2. **问题链推理**:
   - 发现单个问题后，必须尝试构造**影响链**
   - 格式: `问题A (低危) + 问题B (低危) → 组合影响路径 (高危)`
   - 示例: N+1 查询 + 无分页 = 数据量增长后服务不可用
   - 至少探索 2 条可能的影响链路径

3. **安全审计模式**（仅 `--security` / `--ctf` 时激活）:
   - 在信任边界交叉点寻找攻击面
   - 应用攻击者经济学：低成本 + 高收益 = 最先被利用
   - 检查防御深度：≤2 层防御 = 高风险
   - 假设已被入侵：评估横向移动、权限提升、数据外传路径

> 🧠 **Polanyi 内居要求**：如果 `.agent/state/tacit-tradition-map.md` 存在，审查前**必须**加载该文件，寻找代码库的隐性假设作为攻击入口。隐性假设 = 最脆弱的审查面。
