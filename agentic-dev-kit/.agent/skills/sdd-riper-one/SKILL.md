...
1. `create_codemap` — 生成代码地图
2. `build_context_bundle` — 装配上下文包
3. `sdd_bootstrap` — 启动 RIPER（含 Pre-Research 收口）
4. `Research` → `Innovate`(可选) → `Plan` → `Execute` → `Review`

### 快速流（小任务/需求模糊）

1. `sdd_bootstrap` → 按需补 `create_codemap` / `build_context_bundle`
2. `Research` → `Plan` → `Execute` → `Review`
- 门禁：
  - 首版 spec 落盘前，不进入实现
