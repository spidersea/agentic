...
```
Step 0: 解析记忆路径：优先项目级 .agent/memory/；无项目记忆时使用全局 ~/.codex/memories/；只读兼容旧 ~/.codex/memory/
Step 1: 读取当前记忆根目录下的 .dreaming-log.md
...
- 项目级记忆：`.agent/memory/`，优先级最高。
- 全局记忆：`~/.codex/memories/`，仅在当前工作区没有项目级 `.agent/memory/` 时使用。
- 旧路径：`~/.codex/memory/` 只允许读取迁移，不允许继续写入。
...
## 写入前 Self-Critique Gate（P0 · Loop Engineering）

> "A loop with no critic is just an agent nodding along to its own work."
> 写入记忆前，Agent 必须逐条通过三道自审闸门。任一道未通过 → 不写入或降级。

### Gate 1: 反事实检验 · "删了会犯错吗？"
假设这条记忆不存在。下次会话中 Agent 遇到相同场景时——
- 会犯同样的错误？→ **通过**（值得写）
- 会多花时间试错？→ **通过**（写但 Stage ≤ 2）
- 仍能通过推理得出相同结论？→ **不通过**（不值得写）

### Gate 2: Stage 证据检验 · "标签有证据吗？"
- Stage 3 "Verified" → 必须有：真实数据 / 用户确认 / 测试结果至少一项
- Stage 4 "Generalized" → 必须有：2+ 不同场景的验证记录
- 只有"感觉"没有证据 → 降级到 Stage 1-2 或不写

### Gate 3: 重复检验 · "已有记忆覆盖了吗？"
- 扫描已有记忆，是否已有相同或高度重叠的条目？
- 有 → 更新已有条目（提高置信度、追加来源），不新增
- 无 → 通过
