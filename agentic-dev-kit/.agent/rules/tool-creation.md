...
- [ ] **纯读取操作** — 脚本**不得**修改任何源文件、配置文件或状态文件
- [ ] **输出结构化结果** — 脚本输出必须为 JSON、TSV 或 Markdown 格式
- [ ] **Timeout 保护** — 所有脚本执行**必须**包装在 `timeout 30` 中
- [ ] **执行后清理** — 脚本完成后**必须**清理 `.agent/scratch/` 中的临时文件
- [ ] **沙箱隔离** — 脚本**不得**发起网络请求（除非在 `--ctf` 模式下）
- [ ] **语言限制** — 仅允许 bash、python3、node 三种语言
- [ ] **大小限制** — 单个脚本不超过 100 行
