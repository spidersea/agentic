...
0. **运行定量健康检查**
   ...
   - **上下文重量检查**（防止 AGENT.md 和 openspec 积压）：
     ...
     ```
     - AGENT.md > 120 行 → ⚠️ 超重，优先精简路由表（合并同类行、Tier 2 压缩）
...
// turbo
1.5 **会话经验提取（持续学习）**
   扫描近期 checkpoint 文件和 handoff 备忘录，提取实际开发中出现的行为模式：
   ```bash
   # 查找最近的检查点文件
   ...
   ```
   - 阅读这些文件，识别**反复出现的 AI 不良行为**（曾经纠正过 2+ 次的问题）
   - 识别**反复有效的解决模式**（在多个会话中被验证的做法）
   - 对每个候选模式打置信分（1-5）：
     - 5分：同一模式在 3+ 个不同会话中被验证 → **直接升级为正式规则**
     - 3-4分：在 2 个会话中出现 → **列为候选规则，标注 [候选]**
     - 1-2分：仅一次出现 → 记录观察但不生成规则
   - ⚠️ 仅提取**与代码质量和工程流程相关**的模式，不提取项目业务逻辑
