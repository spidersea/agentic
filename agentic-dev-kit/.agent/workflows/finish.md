...
5. **执行选择**
   ...
   **选项 2: 推送并创建 PR**
   - **强制前置动作**：静默运行 `git diff origin/<基础分支>...<功能分支>` 收集真实变更证据
   - 将 diff 数据分析后，自动生成结构化、专业的 PR 摘要
   ```bash
   git push -u origin <功能分支>
   # 如果 gh CLI 可用，将基于真实 diff 自动生成的精准详尽的摘要作为 body 写入
   gh pr create --title "<基于变更自动生成的标题>" --body "## 变更摘要\n<根据 diff 自动生成的具体变更点说明>\n\n## 测试\n- [x] 全量测试通过"
   ```
