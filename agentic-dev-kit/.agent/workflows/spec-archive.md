...
- `openspec/changes/<change-name>/` 目录**必须**存在且包含产物
...
2. **检查完成状态**
   - **必须**读取 `tasks.md`，统计已完成（`[x]`）和未完成（`[ ]`）任务数
   - 如果有未完成任务：
     - ⚠️ **必须**警告用户，但不阻断归档
     - 询问是否继续归档或先完成剩余任务

3. **三维验证**
   **必须**执行三维度结构化检查（不可跳过任何维度）：

   **3.1 完整性 (Completeness)**
   - tasks.md 中的所有任务是否已勾选
   ...
   **3.3 一致性 (Coherence)**
   ...
   - 如有偏差，标记为 WARNING 并**强制**更新 design.md（不可仅标记而不行动）

   **验证结果分类**：
   | 级别 | 说明 | 是否阻断归档 |
   |---|---|---|
   | CRITICAL | 规格承诺的行为未实现 | ✅ **必须**阻断 |
   | WARNING | 实现与产物不一致但功能正常 | ❌ 不阻断（**必须**标记） |
   | SUGGESTION | 潜在改进空间 | ❌ 不阻断 |

4. **合并 Delta Specs**
   - **必须**读取 `openspec/changes/<change-name>/specs/` 中的 delta specs
   ...
   > ⚠️ 合并是智能的（不是简单复制粘贴）。**必须**保留主规格中未被 delta 触及的内容。

5. **执行归档**
   - 生成归档目录名：`openspec/changes/archive/YYYY-MM-DD-<change-name>/`
   - 移动整个变更文件夹到归档目录
   - **必须**确认归档成功（验证目标目录存在且源目录已移除）

// turbo
6. **输出归档报告**
   **必须**向用户展示归档摘要，**禁止**省略任何项：
   - 归档的变更名称和日期
...
7. **后续动作**
   - **必须**检查合并后的主规格文件是否清晰、无矛盾
   - 如有多个活跃变更，询问是否继续归档下一个
   - **强制**执行 `/checkpoint` 记录归档状态（不可跳过）
